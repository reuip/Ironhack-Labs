...
SELECT district_id, count(client_id) as 'No. people per district' FROM client WHERE district_id<10 GROUP BY district_id ORDER BY district_id;
...
SELECT account_id, sum(amount) as "Total owed" FROM loan GROUP BY account_id ORDER BY sum(amount) desc LIMIT 10;
