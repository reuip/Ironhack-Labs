...
#10 In the order table, which account_ids were responsible for orders between order_id 29540 
...
SELECT district_id, count(client_id) as 'No. clients per district' 
FROM client WHERE district_id<10 GROUP BY district_id ORDER BY district_id;

#14 In the card table, how many cards exist for each type? Rank the result starting with the most frequent type.
SELECT type, count(type) as 'Card Count' FROM card GROUP BY type ORDER BY 'Card Count' desc;

#15 Using the loan table, print the top 10 account_ids based on the sum of all of their loan amounts.
SELECT 
    account_id, SUM(amount) AS 'Total owed'
FROM
    loan
GROUP BY account_id
...
SELECT date, count(loan_id) FROM loan WHERE date < 930907 GROUP BY loan_id ORDER BY date desc;
...
SELECT `date`, duration, count(loan_id) as "Number of loans" FROM loan
WHERE CAST(`date` as date) between "1997-12-01" and "1997-12-31"
...
*/
SELECT CASE when type="PRIJEM" then "Incoming"
when type="VYDAJ" then "Outgoing"
else type end
from trans;

SELECT account_id, type as transaction_type, floor(sum(amount)) as total_amount
FROM trans
WHERE account_id = 396
GROUP BY type;
...
With t1 as (Select floor(sum(amount)) as amt from trans where (account_id=396 and type="incoming")), 
t2 as (Select floor(sum(amount)) as amt from trans where (account_id=396 AND type="outgoing")) 
SELECT t1.amt as "incoming sum",t2.amt as "outgoing sum",(t1.amt-t2.amt) as difference FROM t1 JOIN t2;


#21 Continuing with the previous example, rank the top 10 account_ids based on their difference.
With t1 as (Select account_id, sum(amount) as amt from trans where type="incoming" GROUP BY account_id), 
t2 as (Select account_id, sum(amount) as amt from trans where type="outgoing" GROUP BY account_id) 
SELECT t1.account_id,(t1.amt-t2.amt) as difference
FROM t1 JOIN t2 using (account_ID)
